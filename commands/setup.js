module.exports = {
    ...
    args: true,
    guildOnly: true,
    description: 'Setup bot for current guild `!setup <timezone> <raid-chan> <notifications(yes|no)>`',
    execute(message, args) {
        message.mysql.query('SELECT * FROM bot_oo_ds.guild_list as gl WHERE gl.discord_id = ' + message.guild.id + ' LIMIT 1;', (err, result) => {
            ...
            //*/
            let sql = "UPDATE bot_oo_ds.bot_setup set timezone = ?, raid_chan = ?, notifications = ? WHERE discord_id = ?";
            let inserts = [args[0], chan.id , notif, message.guild.id];
